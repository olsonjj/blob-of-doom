...
- [ ] `scripts/` directory is no longer excluded from linting (either included in main config or given its own relaxed config)
- [ ] `format:check` is blocking in the Vercel build command (no `|| true`)
- [ ] A `.github/workflows/ci.yml` exists that runs `typecheck`, `lint`, `format:check`, and `test` on push and PR
- [ ] All six `"latest"` version strings in `package.json` are replaced with explicit `^` semver ranges matching the currently resolved versions
- [ ] `pnpm install` produces an identical lockfile after the version string changes
- [ ] `.env.example` is not ignored by git (`.gitignore` has `!.env.example` before the `.env*` rule)
- [ ] `nitro` is pinned to a stable release or the beta is documented with a reason
- [ ] TypeScript and typescript-eslint versions are compatible (either downgrade TS to ^5.x or upgrade typescript-eslint)
