...
import { queryFlaggedBlobs, approveFlaggedBlobById } from './admin.func'

// ── Helpers ─────────────────────────────────────────────────────────────────

function makeFlaggedRow(overrides: Partial<{
  id: number
  title: string
...
describe('queryFlaggedBlobs', () => {
  ...
  it('falls back to "Unknown" when Clerk user not found', async () => {
    selectOrderByMock.mockResolvedValueOnce([
      makeFlaggedRow({ id: 1, clerkUserId: 'user_missing' }),
    ])
  ...
  it('handles null moderation scores', async () => {
    selectOrderByMock.mockResolvedValueOnce([
      makeFlaggedRow({ id: 1, moderationScores: null }),
    ])
