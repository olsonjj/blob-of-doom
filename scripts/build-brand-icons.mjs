...
await sharp(svg, { density: 384 }).resize(512, 512).png({ compressionLevel: 9 }).toFile('public/logo512.png');

await sharp(svg, { density: 192 }).resize(192, 192).png({ compressionLevel: 9 }).toFile('public/logo192.png');
...
// ICONDIR header
ico.writeUInt16LE(0, off);
off += 2; // reserved
ico.writeUInt16LE(1, off);
off += 2; // type = icon
ico.writeUInt16LE(sizes.length, off);
off += 2; // count
...
for (let i = 0; i < sizes.length; i++) {
  ...
  ico.writeUInt16LE(1, off);
  off += 2; // planes
  ico.writeUInt16LE(32, off);
  off += 2; // bpp
  ico.writeUInt32LE(b.length, off);
  off += 4; // size
  ico.writeUInt32LE(imgOffset, off);
  off += 4; // offset
  imgOffset += b.length;
}
